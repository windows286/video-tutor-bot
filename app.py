...
# 6. 사용자 질문 입력 및 답변 출력
if prompt := st.chat_input("질문을 입력하세요 (예: 가장 기본 숏 3가지는?)"):
    with st.chat_message("user"):
        st.markdown(prompt)

    with st.chat_message("assistant"):
        with st.spinner("조교가 답변을 생각하고 있습니다..."):
            try:
                response = st.session_state.chat_session.send_message(prompt)
                st.markdown(response.text)
            except Exception as e:
                st.error(f"답변 생성 중 오류가 발생했습니다. API 키나 모델 설정을 확인해주세요. 상세 에러: {e}")
